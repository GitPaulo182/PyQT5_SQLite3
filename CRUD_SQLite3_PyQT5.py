...
class Janela3 (QMainWindow):
    ...
    # CARACTERÍSTICAS DA QUARTA LINHA
    ...
    #FUNÇÃO PARA ATUALIZAÇÃO DA LINHA SELECIONADA
    def atualizarLinha(self):
        ...
        try:
            ...
            numero_id = self.linha4a.text()
            params = (nome1, idade1, email1, numero_id)
            banco = sqlite3.connect('banco_dados01.db')
            cursor = banco.cursor()
            cursor.execute(f"UPDATE pessoas SET nome = ? , idade = ?, email= ? WHERE id = ?", params)
            banco.commit()
            banco.close()
...
class Janela2(QMainWindow):
    ...
    def terceiroBotao(self):
        self.botao3 = QPushButton("Tela Cadastro", self)
        self.botao3.move(170, 500)
        self.botao3.resize(100, 30)
        self.botao3.clicked.connect(self.retornaJanelaPrincipal)
